...
void buscaTabu() {
    ...
    double lista_tabu[quant_dep][quant_dep];
    int tempo_tabu = 0;
    int min_tabu = quant_dep / 3;
    int max_tabu = (3 * quant_dep) / 4;
        for(i = 0; i < quant_dep; i++){
            for(j = 0; j < quant_dep; j++){
                lista_tabu[i][j] = -1;//n esta na lista
            }
        }
    double valor_mov;
    ...
    for (iter_atual=1; iter_atual<=max_iteracoes; iter_atual++) {
        ...
        if (melhor_valor<INF && melhor_valor<valor_func) {
            printf("*** %lf\n", melhor_valor);
            if(lista_tabu[dep_esc][pos_esc] == -1 && lista_tabu[pos_esc][dep_esc] == -1){
                fazer_mudanca(dep_esc, pos_esc);
                tempo_tabu = min_tabu + (min_tabu + rand() % max_tabu);//tempo tabu (tt)
                lista_tabu[dep_esc][pos_esc] = iter_atual + tempo_tabu;//add lista
                lista_tabu[pos_esc][dep_esc] = iter_atual + tempo_tabu;//add lista
            }else{
                if((lista_tabu[dep_esc][pos_esc] != -1 && lista_tabu[pos_esc][dep_esc] != -1) && aspiracao(valor_func, avmudanca )){
                    lista_tabu[dep_esc][pos_esc] = iter_atual + tempo_tabu;//add lista
                    lista_tabu[pos_esc][dep_esc] = iter_atual + tempo_tabu;//add lista
            }
